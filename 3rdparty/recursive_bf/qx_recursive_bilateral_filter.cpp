...
inline double qx_compute_color_difference_double(double a[3], double b[3])
{
	double dr=abs(a[0]-b[0]);
	double dg=abs(a[1]-b[1]);
	double db=abs(a[2]-b[2]);
	return(((dr/2.0)+dg+db)*4.0);
}
...
void qx_recursive_bilateral_filter_2chan(double***out,double***in,double***texture,double sigma_spatial,double sigma_range,int h,int w,double***temp,double***temp_2w,double**factor,double**temp_factor,double**temp_factor_2w)
{
	int QX_LOOKUP_TABLE_NUMPTS = 10000;
	int MAX_DIST = 256;
	double range_table[QX_LOOKUP_TABLE_NUMPTS+1];//compute a lookup table
	double range_ratio = ((double)QX_LOOKUP_TABLE_NUMPTS)/((double)MAX_DIST);

	double inv_sigma_range = 1.0/(sigma_range*QX_LOOKUP_TABLE_NUMPTS);
	double range_step = inv_sigma_range;
	for(int i=0;i<=QX_LOOKUP_TABLE_NUMPTS;i++) { range_table[i]=exp(-i*inv_sigma_range); }
	// range_table(i) = exp(-(i/N)/sigma ), i=0,...,N

	double alpha=exp(-sqrt(2.0)/(sigma_spatial*w));//filter kernel size
	double***in_=in;
	double tpr,tpg,tpb,tcr,tcg,tcb;
	double ypr,ypg,ypb,ycr,ycg,ycb;
	double fp,fc;//factor
	double inv_alpha_=1-alpha;
	for(int y=0;y<h;y++)/*horizontal filtering*/
	{
		...
		*temp_factor_x++=fp=1; 
		...
		{
			tcr=*texture_x++; tcg=*texture_x++; tcb=*texture_x++;
			double dr=abs(tcr-tpr);
			double dg=abs(tcg-tpg);
			//double db=abs(tcb-tpb);

			double range_dist = (dr+dg);
			int idx = min( QX_LOOKUP_TABLE_NUMPTS, (int)(range_dist * range_ratio) );

			double weight=range_table[idx];
			double alpha_=weight*alpha;
			*temp_x++=ycr=inv_alpha_*(*in_x++)+alpha_*ypr; *temp_x++=ycg=inv_alpha_*(*in_x++)+alpha_*ypg; *temp_x++=ycb=inv_alpha_*(*in_x++)+alpha_*ypb;//update temp buffer
			tpr=tcr; tpg=tcg; tpb=tcb;
			ypr=ycr; ypg=ycg; ypb=ycb;
	...
	for(int y=1;y<h;y++)
	{
		...
		for(int x=0;x<w;x++)
		{
			double dr=abs((*tcy++)-(*tpy++));
			double dg=abs((*tcy++)-(*tpy++));
			double db=abs((*tcy++)-(*tpy++));
