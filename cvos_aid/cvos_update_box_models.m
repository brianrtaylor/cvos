%-----------------------------------------------------------------------------
% cvos_update_box_models
%
% updates the colour and shape models in the local shape classifiers and
% creates new ones as needed
%
% @param: boxes: existing local shape classifiers
% @param: layers (MxNxL): layer segmentation from the current frame
% @param: img (MxNx3): input image (typically the bilateral filtered image)
% @param: Dx, Dy: difference operators in x and y directions
% @param: opts: options struct
%-----------------------------------------------------------------------------
function boxes = cvos_update_box_models(boxes, layers, img, Dx, Dy, opts)
  ...
  if max(layers(:)) >= 1;       
    ...
    if n_newboxes > 0;
      [mu_fg, cov_fg, pi_fg, mu_bg, cov_bg, pi_bg] = learn_bbox_gmm_mex( ...
        img, double(layers), new_boxes, CP.NUM_GMM_CLUSTERS, ...
        CP.NUM_GMM_REPETITIONS, CP.NUM_GMM_ITERATIONS, ...
      ...
      [box_fg, box_bg] = eval_gmm_bboxes_mex(img, new_boxes);
      for bk = 1:n_newboxes;
    ...
    if n_boxes > 0; % for old boxes, do some checks
      [mu_fg, cov_fg, pi_fg, mu_bg, cov_bg, pi_bg] = learn_bbox_gmm_mex( ...
        img, double(layers), boxes, CP.NUM_GMM_CLUSTERS, ...
        CP.NUM_GMM_REPETITIONS, CP.NUM_GMM_ITERATIONS, ...
        GMM_PROB_FG_MASK_THRESH, GMM_PROB_BG_MASK_THRESH);
      ...
      [box_fg_1, box_bg_1] = eval_gmm_bboxes_mex(img, boxes);
      ...
      for bk = 1:n_boxes;
        ...
        up_boxes(bk).fg_gmm_pi = up_boxes(bk).fg_gmm_pi ...
          / sum(up_boxes(bk).fg_gmm_pi);
        ...
        up_boxes(bk).bg_gmm_pi = up_boxes(bk).bg_gmm_pi ...
          / sum(up_boxes(bk).bg_gmm_pi);
      ...
      [box_fg_2, box_bg_2] = eval_gmm_bboxes_mex(img, up_boxes);
  
      % select historic or updated model
      for bk = 1:n_boxes;
