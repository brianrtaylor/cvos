...
%   cross_bilateral_filter_step_occ(occb, occf, uvb, uvf, uvb_rev, uvf_rev, I0, I1, I2, m);
%
...
% function [uvb_cbf, uvf_cbf, occb_cbf, occf_cbf, occb_cbf_prob, occf_cbf_prob] = cross_bilateral_filter_step_test_occ( ...
%   occb, occf, uvb, uvf, occb_rev, occf_rev, uvb_rev, uvf_rev, I0, I1, I2, m)
...
if ~exist('VIS', 'var'); VIS = 100; end;
  ...
  %-------------------------------------------------------------------
  % f_change = imdilate(occf_prob, dsk);
  % b_change = imdilate(occb_prob, dsk);
  ...
  if (isnan(uvb_avg) || isnan(uvf_avg));
    ...
    % whole image filtering status
    % atic = tic();
    % uvf_cbf_2 = cbf_flow(uvf_fun_2, uvb_fun_2, f_change, ...
    %   occf_fun_prob_2, occb_fun_prob_2, W, sigma_r_f, sigma_r_b, 2*uvf_avg);
    % atic = toc(atic); fprintf('uvb cbf 2 (%0.3f s)\n', atic);

    % atic = tic();
    % uvb_cbf_2 = cbf_flow(uvb_fun_2, uvf_fun_2, b_change, ...
    %   occb_fun_prob_2, occf_fun_prob_2, W, sigma_r_b, sigma_r_f, 2*uvb_avg);
    % atic = toc(atic); fprintf('uvf cbf 2 (%0.3f s)\n', atic);

    % atic = tic();
    % uvf_cbf_2 = cbf_flow(uvf_fun, uvb_fun, f_change, ...
    %   occf_fun_prob, occb_fun_prob, W, sigma_r_f, sigma_r_b, 2*uvf_avg);
    % atic = toc(atic); fprintf('uvb cbf 2 (%0.3f s)\n', atic);

    % atic = tic();
    % uvb_cbf_2 = cbf_flow(uvb_fun, uvf_fun, b_change, ...
    %   occb_fun_prob, occf_fun_prob, W, sigma_r_b, sigma_r_f, 2*uvb_avg);
    % atic = toc(atic); fprintf('uvf cbf 2 (%0.3f s)\n', atic);
  end
  ...
  % fig(243); imagex([[uvb, uvf];[uvb_filt, uvf_filt];[uvb_fun, uvf_fun]; ...
  %   [uvb_fun_2, uvf_fun_2];[uvb_cbf, uvf_cbf]]);
  % title('uvb / filt / fun / fun-2 / cbf | uvf / filt / fun / fun-2 / cbf '); colorbar; drawnow; 
  %-------------------------------------------------------------------
...
%   occf_cbf_prob = occ_res_to_prob(occf_cbf, true);
  ...
  % occb_cbf_prob_2  = max(0.0, occb_cbf_prob_2 - occf_cbf_prob_2 - occ_res_to_prob(occf_rev));
  % occf_cbf_prob_2  = max(0.0, occf_cbf_prob_2 - occb_cbf_prob_2 - occ_res_to_prob(occb_rev));
  occb_cbf_prob_2  = max(0.0, occb_cbf_prob_2 - occf_cbf_prob_2);
  occf_cbf_prob_2  = max(0.0, occf_cbf_prob_2 - occb_cbf_prob_2);

  % TODO: should try this, with no cbf (faster)
%   occb_cbf_prob_2_ = occ_res_to_prob(max(0.0, occb_0), true);
%   occf_cbf_prob_2_ = occ_res_to_prob(max(0.0, occf_0), true);
  % occb_cbf_prob_2_ = occ_res_to_prob(max(0.0, occb_cbf_2 - occf_cbf_2));
  % occf_cbf_prob_2_ = occ_res_to_prob(max(0.0, occf_cbf_2 - occb_cbf_2));
%   occb_cbf_prob_2_ = max(0.0, occb_cbf_prob_2_ - occf_cbf_prob_2_);
%   occf_cbf_prob_2_ = max(0.0, occf_cbf_prob_2_ - occb_cbf_prob_2_);
  % occb_cbf_prob_2_ = max(0.0, occb_cbf_prob_2_ - occf_cbf_prob_2_ - occ_res_to_prob(occf_rev, true));
  % occf_cbf_prob_2_ = max(0.0, occf_cbf_prob_2_ - occb_cbf_prob_2_ - occ_res_to_prob(occb_rev, true));
  % occb_cbf_prob_2_ = occb_cbf_prob_2_ .* (1 - occf_cbf_prob_2_);
  % occf_cbf_prob_2_ = occf_cbf_prob_2_ .* (1 - occb_cbf_prob_2_);

  %-------------------------------------------------------------------
  % visuals and output
  %-------------------------------------------------------------------
%   if VIS < 150;
%     fig(246); imagesc(min(70, [ ...
%       [occb_filt; occb_fun_2; occb_cbf; occb_cbf_sole; occb_0], ...
%       [occf_filt; occf_fun_2; occf_cbf; occf_cbf_sole; occf_0]]));
%     title(['occb -- filt / fun-2 / cbf / sole / 0 '...
%       'occf -- filt / fun-2 / cbf / sole / 0 ']); colorbar; drawnow;
%     fig(245); imagesc([ ...
%       [occb_filt_prob; occb_fun_prob_2; occb_cbf_prob; occb_cbf_prob_2; occb_cbf_prob_2_], ...
%       [occf_filt_prob; occb_fun_prob_2; occf_cbf_prob; occf_cbf_prob_2; occf_cbf_prob_2_]]);
%     title(['occb -- filt-prob / fun-prob-2 / cbf-prob / cbf-prob-2 / cbf-prob-2- | ' ...
%       'occf -- filt-prob / fun-prob-2 / cbf-prob / cbf-prob-2 / cbf-prob-2-']);
%     colorbar; drawnow;
%   end
