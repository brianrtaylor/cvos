% function prob = bboxes_to_prob_image(imsize, ...
%   bb_center, bb_r, stub_prob, stub_conf)

% NOTE: CURRENTLY ASSUMES ALL BOXES ARE THE SAME SIZE, BUT WE CANNOT AND
% DON'T WANT TO ASSUME THIS IN GENERAL
...
for k = 1:n_boxes;
  ...
  % note: ignore conf here since conf_colour affects conf shape, do it with
  % input arguments for now
  ...
  prob(ys, xs) = new_prob;
  % prob(ys, xs) = prob(ys, xs) + bb_prob(bys,bxs,k);
