...
void apply_dy_matlab_sse(float* out, const float* in, int rows, int cols) { 
    ...
    for (int c = 0; c < cols; ++c) { 
        ...
        for (int r = 0; r < nloops_r; ++r) { 
            m_in1 = _mm_loadu_ps( in + (4*r+c*rows) );
            m_in2 = _mm_loadu_ps( in + (1+4*r+c*rows) );
            m_out = _mm_sub_ps(m_in2, m_in1);
            _mm_storeu_ps( out + (4*r+c*rows), m_out );
        }
        // remainder: 
...
void apply_dyt_matlab_sse(float* out, const float* in, int rows, int cols) { 
    ...
    for (int c = 0; c < cols; ++c) { 
        ...
        for (int r = 1; r < nloops_r; ++r) { 
            m_out  = _mm_loadu_ps( out + (4*r+c*rows) );
            m_in1  = _mm_loadu_ps( in + (4*r+c*rows) );
            m_in0  = _mm_loadu_ps( in + (4*r+c*rows-1) );
            m_temp = _mm_sub_ps(m_in0, m_in1);
            m_out  = _mm_add_ps(m_out, m_temp);
            _mm_storeu_ps( out + (4*r+c*rows), m_out );
            //! NOTICE THAT THIS IS '+=', rather than '='.
...
void apply_dx_matlab_sse(float* out, const float* in, int rows, int cols) { 
    ...
    for (int c = 0; c < cols; ++c) { 
        ...
        for (int r = 0; r < nloops_r; ++r) { 
            m_in1 = _mm_loadu_ps( in + (4*r+c*rows) );
            m_in2 = _mm_loadu_ps( in + (4*r+cn*rows) );
            m_out = _mm_sub_ps(m_in2, m_in1);
            _mm_storeu_ps( out + (4*r+c*rows), m_out );
        }
        // remainder:
...
void apply_dxt_matlab_sse(float* out, const float* in, int rows, int cols) {
    ...
    for (int c = 0; c < cols; ++c) {
        ...
        for (int r = 0; r < nloops_r; ++r) { 
            m_in1 = _mm_loadu_ps( in + (4*r+c*rows) );
            m_in0 = _mm_loadu_ps( in + (4*r+cp*rows) );
            m_out = _mm_sub_ps(m_in0, m_in1);
            _mm_storeu_ps( out + (4*r+c*rows), m_out );
        }
        for (int r = nloops_r*4; r < rows; ++r) { 
