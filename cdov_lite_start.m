%----------------------------------------------------------------------------%
% cdov_lite_start(params, ADD, k)
%
% causal detachable objects for a single frame (i.e. first and last frames)
%
% @note: so far just does the first frame lol
%----------------------------------------------------------------------------%
function cdov_lite_start(params, ADD, k)
setup;
...
[uvb, uvb_cbf, uvf, uvf_cbf, uvb_rev, uvf_rev, ...
  ...
  tao_flow_occ(flow_path, flow_files, seq, kk, FINISH, ...
...
object_mean_uvf_map = tao_get_object_flow_maps(objects, ADD.object_map);
...
[past.t0.layers, layers_t0_msfm] = remove_occb_mask( ...
  layers_t0, occf_mask, 0, 0); % past = t+1 lol
...
% constraints now
%------------------------------------------------------------------
% [constraints_now_b, constraint_weights_now_b, indbad_now_b] = deal([], [], []);
...
if ~isempty(past.unity) && params.UNITYHELP && params.CAUSAL ...
  && params.PROB_UNITY > 0;
  unity = tao_unity_prior(past, uvf_bflt, uvf_cbf_bflt, ...
...
if ~isempty(past.prob_fg) && params.CAUSAL && params.PROB_FG > 0;
  prob_fg = tao_fg_prior(past, uvf_cbf_bflt, occf_mask, opts_fg);
else
