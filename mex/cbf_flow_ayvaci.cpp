...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
  ...
  // show gaussian for debugging
  int wm = floor(2*w_+1);
  int gwidx;
  double* w_dist = new double[wm * wm];
  double x2, y2;
  for (int xx = 0; xx < wm; ++xx) {
    for (int yy = 0; yy < wm; ++yy) {
      gwidx = linear_index(yy, xx, wm);
      x2 = (double) (xx - w_) * (double) (xx - w_);
      y2 = (double) (yy - w_) * (double) (yy - w_);
      w_dist[gwidx] = x2 + y2;
    }
  }
  if (nlhs > 1) { // just example output
    double sigma_loc_sq_inv = 1.0/(2.0*sigma_loc*sigma_loc);
    double* g_show = new double[wm * wm];
    for (int xx = 0; xx < wm; ++xx) {
      for (int yy = 0; yy < wm; ++yy) {
        gwidx = linear_index(yy, xx, wm);
        float v = w_dist[gwidx] * sigma_loc_sq_inv;
        int exp_idx = min( int( ((v)/EXP_MAX_) *EXP_NUM_), EXP_NUM_-1);
        g_show[gwidx] = exp_[exp_idx];
        // g_show[gwidx] = v;
      }
    }

    plhs[1] = mxCreateDoubleMatrix(wm, wm, mxREAL);
    double* gdists = (double*) mxGetPr(plhs[1]);
    memcpy(gdists, g_show, sizeof(double)*wm*wm);
  }
    ...
    for (int y = 0; y < rows; ++y) {
      ...
      if ((m_o > 0.1f)) {
        ...
        //if (energy_uvb_local > energy_uvf_local) {
          ...
          // uvf_[idx1] = uvf[idx1] * (1.0 - m_o) + val_x * m_o;
          // uvf_[idx2] = uvf[idx2] * (1.0 - m_o) + val_y * m_o;
          // mexPrintf("changed (x=%d,y=%d)\n", x, y);
          uvf_[idx1] = val_x;
          uvf_[idx2] = val_y;
        // }
        // mexPrintf("filter_norm: %0.3f\n", filter_norm);
      }
    }
  }
  // fin.
